/* gcc xsetpointer2.c -I /usr/include/X11 -lX11 -lXi */
...
main(int argc, char * argv[])
{
  ...
  if (argc < 2 || argc > 3) {
    ...
    fprintf(stderr, "usage : %s ( -C | +C ) <device id>)\n", argv[0]);
    exit(1);
  }
	  ...
	  if (list) {
		  ...
		  break;
            ...
            if (argc == 3 && devices[loop].name &&
                (    ( (!by_id) && StrCaseCmp(devices[loop].name, argv[2]) == 0 )
		  || ( ( by_id) && (devices[loop].id == id))) ) {
#ifdef DEBUG
              ...
              if (device) {
		ectl.enable    = (core - 1);
		ectl.length = sizeof(ectl);
		ectl.control = DEVICE_ENABLE;
                XChangeDeviceControl(dpy, device, DEVICE_ENABLE,
                                     (XDeviceControl *)&ectl);
                exit(0);
              }
              else {
  ...
  if (list) {
    ...
    if (by_id)
      fprintf(stderr, "Extended device id %d not found\n", id);
    else
