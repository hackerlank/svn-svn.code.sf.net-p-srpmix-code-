...
(define (href-for dir entry err-return)
  (let ((path (simplify-path (build-path dir entry)))
	(prefix-length (string-length prefix)))
    (cond
     ((string-prefix? prefix path)
      (html-escape-string (format "http://srpmix.org/api/browse.cgi?path=~a&display=font-lock"
				  (wash-path (string-drop path prefix-length)))))
     (else
      (href-for prefix "." err-return)))))
...
(define (build-line entry dir? marked? href symlink?)
	   ...
	   (html:body
	    (html:pre 
	     (list
	      ...
	      (build-line "." #t #f (href-for path  "." err-return)   #f)
	      (build-line ".." #t #f (href-for path  ".." err-return) #f)
	      
	      (map (lambda (e)
		     (let1 dir? (file-is-directory? (build-path path e))
		       (cond
			((member e '("STATUS"))
			 ;; TODO: Use status with face
			 (build-line e dir? #t (href-for path e err-return) #f)
			 )
			
			(else
			 (let1 symlink (symlink-for path e)
			     (build-line e dir? #f (href-for path e err-return) symlink))
			 ))))
		   files))))))))
